...
Protect optimisation from rare destructive updates and distinguish clipping from a cure for poor modeling. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Global norm clipping replaces g by g·min(1,c/||g||). Repeated matrix products can explode when effective spectral magnitudes exceed one; clipping bounds update size but does not create long-term memory.
...
1) Clip g=(3,4) at c=2. 2) Compare value versus norm clipping. 3) Trace repeated multiplication by 1.2. 4) Interpret a clip-rate log.
...
Add global-norm clipping to a training loop; log pre-clip norm, post-clip norm, and clipping frequency. Investigate one spike’s input batch. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Hiding persistent instability with an extremely low threshold; inspect learning rate, data corruption, and architecture too.
...
For g=(3,4), norm 5 clipped at 2 becomes (1.2,1.6). Two tensor norms 3 and 4 have global norm 5, not 7. Repeated 1.2^20≈38.3 while .8^20≈.012, illustrating exploding and vanishing products. A robotic controller needs a guardrail against one anomalous sensor batch. Debug clipping after the update (useless), clip rates near 100% (systemic problem), and mixed-precision unscaling order.

...
track: "deep-learning"
status: live
summary: "Noise prediction, schedule choice, sampler choice, and guidance jointly determine diffusion behavior."
...
A common objective is $\mathbb E_{t,x_0,\epsilon}\|\epsilon-\epsilon_\theta(x_t,t,c)\|^2$. Classifier-free guidance combines conditional and unconditional estimates.
...
Guided noise may be $\epsilon_g=\epsilon_u+w(\epsilon_c-\epsilon_u)$. Increasing w improves condition adherence but can reduce diversity or create artifacts.
...
If $\epsilon_u=1$, $\epsilon_c=.7$, and $w=3$, calculate $\epsilon_g=.1$; note this extrapolates past the conditional estimate.
...
For a design tool, provide seed, prompt, model version, and sampler settings so a result can be reproduced and investigated.
...
Fast samplers use fewer reverse steps; compare quality and safety on the same prompts rather than using a favorable cherry-picked gallery.

...
order: 213
status: live
summary: "Treat expert knowledge as a versioned, testable artifact rather than a one-time interview transcript."
...
A proposed rule can be checked against known cases: count how many expected decisions it changes, inspect the changed cases, then obtain owner sign-off.
...
Experts may disagree because the world is ambiguous, not because one person is wrong. Preserve disagreement instead of forcing false precision.

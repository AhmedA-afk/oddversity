...
track: "deep-learning"
status: live
summary: "Embeddings make similarity computable, but geometry inherits the supervision, sampling, and bias of training."
...
For normalized embeddings, similarity is $s(q,d)=q^\top d$. Contrastive loss increases a positive score relative to negatives: $-\log\frac{e^{s(q,d^+)/\tau}}{\sum_j e^{s(q,d_j)/\tau}}$.
...
Temperature $\tau$ controls the sharpness of the contrast. Hard negatives create strong signal but can be false negatives that damage useful equivalence.
...
For vectors $(1,0)$ and $(.6,.8)$, cosine similarity is $.6$; normalization removes magnitude as an accidental relevance cue.
...
In product search, two items can be visually similar yet incompatible; train and evaluate on the relevance definition, not aesthetic closeness.
...
Nearest neighbors from one demographic cluster may reveal representation bias; audit retrieval exposure as well as classification metrics.

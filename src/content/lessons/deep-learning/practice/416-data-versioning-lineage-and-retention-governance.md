...
Represent provenance as a directed acyclic graph (raw\rightarrow cleaned\rightarrow split\rightarrow run\rightarrow artifact\rightarrow deployment). A change impact set is every descendant of a changed raw record or policy. Retention is a time-bounded rule over that graph, not a promise attached only to raw storage.
...
A corrected label affects a validation slice and a released model card; lineage identifies both.
...
A consent withdrawal requires removal from a cache and determines whether a retraining run is needed.
...
A vendor changes license terms; stored manifest proves which corpus version was used.
...
- **Symptom → likely cause → first check:** A consent withdrawal requires removal from a cache and determines whether a retraining run is needed. Inspect the smallest deterministic fixture before changing hyperparameters.
...
Data lineage ties an output back to source, transformation, policy, and retention status. A dataset version should contain immutable manifest identifiers, transform version, split version, label policy, and access controls—not only a mutable storage path.

## Worked scenario

If a user requests deletion, identify every derivative governed by policy: raw record, feature cache, training shard, evaluation set, and model retrain schedule. If a source label is corrected, quantify affected examples and releases.
...
Build a lineage table for raw source through model artifact. Simulate a label correction and decide whether it triggers a retrain, evaluation-only update, or incident review.

## Failure modes to diagnose

A hash of a CSV is not full lineage if extraction criteria are missing. Keeping data forever “for reproducibility” can violate retention promises.

## Decision standard

Make governance decisions executable through manifests, access logs, and deletion/retraining playbooks.

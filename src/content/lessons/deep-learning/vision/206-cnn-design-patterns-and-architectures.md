...
status: live
order: 206
summary: "Compare LeNet, AlexNet, VGG, Inception, ResNet, and efficient CNN choices by constraints."
duration: "26 min read"
---

Architecture history is a record of constraints: LeNet made local learned features
practical; VGG used repeated 3x3 layers; Inception used multi-scale branches; ResNet
made very deep optimization tractable; MobileNet traded dense convolution for
depthwise-separable computation.

A standard 3x3 convolution costs roughly (HWC_{in}C_{out}9) multiply-adds.
Depthwise plus pointwise costs (HWC_{in}(9+C_{out})). With 128 input/output
channels, the ratio is ((9+128)/(9cdot128)approx0.119): about 8.4x fewer
operations, but accuracy and hardware latency must be measured, not assumed.

Use VGG-like regularity for teaching and controlled baselines; use residual networks
for reliable supervised backbones; use mobile families under strict edge budgets.
Multi-branch modules help when objects vary in scale, but branch complexity can impede
debugging and deployment.

```python
block = nn.Sequential(nn.Conv2d(c, c, 3, padding=1), nn.BatchNorm2d(c), nn.ReLU())
```

## Diagnostic comparison

Hold data, augmentation, optimizer, epochs, and resolution fixed. Log parameters,
FLOPs, measured latency, peak memory, calibration, and class-wise error—not just top-1.
If a mobile model is slower, profile actual operators; depthwise kernels can be
hardware-unfriendly at small batch sizes.

## Assignment

Implement a VGG-style baseline and a depthwise-separable variant with matched output
shapes. Calculate parameters for a 64-to-128 block in each. Run three scenarios:
server batch 64, phone batch 1, and a small-data task. Make a deployment recommendation
with evidence and name one risk that no architecture choice addresses.


## Advanced studio: calculation, implementation, and decision evidence

The compact explanation above is only a starting point. To master **cnn design patterns and architectures**, make

...
track: "deep-learning"
status: live
summary: "Caching past keys and values turns repeated-prefix attention into incremental decoding, but memory becomes the bottleneck."
...
At generation step $t$, cache $K_{1:t},V_{1:t}$ and compute only new $q_t,k_t,v_t$. Per-token attention remains $O(t)$, but recomputation of prior projections disappears.
...
For 32 layers, 32 heads, head dimension 128, and 4,000 tokens, estimate cache elements: $2\times32\times4000\times4096$. Convert to bytes for your dtype.
...
Batching requests with different lengths improves throughput but can worsen tail latency; track both tokens/s and p95 latency.
...
A cache indexed with the wrong request order causes fluent but cross-user-contaminated output: test isolation explicitly.

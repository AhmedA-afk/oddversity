...
status: live
order: 215
summary: "Design decoders that recover spatial detail without inventing precise-looking errors."
duration: "24 min read"
---

A decoder upsamples semantic features; it cannot recover evidence discarded by the
encoder. Transposed convolution can learn upsampling but uneven kernel/stride overlap
causes checkerboard artifacts. For (K=3,S=2), some output positions receive more
input contributions than others. Resize-convolution (interpolate then convolve) is
often easier to reason about.

U-Net concatenates encoder detail with decoder context. If a 64-channel decoder joins
64 skip channels, the next 3x3 convolution to 64 channels has
((128cdot64cdot9)+64=73,792) parameters. Addition is cheaper but assumes aligned
semantics and equal channels.

Scenario: boundary labels from different annotators vary more than model predictions.
Scenario: reflect padding helps edge texture but can manufacture mirrored anatomy.
Scenario: a decoder predicts smooth masks because low-resolution logits were simply
bilinearly enlarged.
...
Build two decoders: transpose-convolution and resize-convolution. Feed an impulse and
checkerboard, then save outputs to identify artifacts. Measure region Dice and boundary
F-score on a thin-object set. Write a design memo explaining crop/pad conventions,
alignment tests, and when a clinician/operator should see an uncertainty contour rather
than a binary edge.


## Advanced studio: calculation, implementation, and decision evidence

The compact explanation above is only a starting point. To master **segmentation decoders and boundaries**, make

...
Use momentum to damp ravines and distinguish velocity conventions correctly. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Classical momentum v_t=μv_{t−1}+g_t; θ_t=θ_{t−1}−ηv_t. Along a consistent gradient, velocity accumulates; across alternating gradients it cancels. Nesterov evaluates near the look-ahead point.
...
1) Unroll two momentum steps. 2) Compare μ=0 and .9. 3) Analyse a narrow quadratic valley. 4) Explain why learning-rate scaling matters when changing conventions.
...
Optimise an ill-conditioned quadratic with SGD and momentum; plot parameter trajectories and overshoot at three learning rates. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Copying hyperparameters between implementations whose momentum definitions differ; check the exact update before tuning.
...
With μ=.9, scalar gradients 1,1 give velocity 1 then 1.9; η=.1 moves -.1 then -.19. Alternating gradients 1,-1 give v2=-.1, showing damping. On 100x^2+y^2, equal SGD steps oscillate in x while momentum smooths persistent direction. A speech model resumed after a data change needs optimizer state awareness. Debug convention mismatches: libraries define Nesterov updates differently, so inspect one exact step before copying hyperparameters.

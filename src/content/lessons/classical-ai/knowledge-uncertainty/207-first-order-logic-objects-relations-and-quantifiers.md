...
order: 207
status: live
summary: "Represent reusable relations among objects without accidentally changing who or what a rule refers to."
...
`∀x (sensor(x) → needs_calibration(x))` says every sensor needs calibration, while `∃x sensor(x)` merely says one exists. Quantifier order changes the claim.
...
Do not use a universal rule to smuggle in an existence claim, and do not assume names uniquely identify real-world entities.

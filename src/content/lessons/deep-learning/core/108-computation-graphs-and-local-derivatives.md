...
status: live
order: 108
description: "Represent a model as primitive operations whose local derivatives compose mechanically."
---

# Computation Graphs and Local Derivatives

## Why this matters

Represent a model as primitive operations whose local derivatives compose mechanically. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

For q=(ab+c)², graph nodes are multiply, add, square. Reverse accumulation gives q̄=1, s̄=2s, ā=s̄b, b̄=s̄a, c̄=s̄. The bar denotes ∂q/∂node.
...
1) Trace q at a=2,b=3,c=1. 2) Compute all adjoints. 3) Reuse a variable in x²+x and show gradient addition. 4) Draw a branch whose two paths meet.
...
Draw and differentiate a loss `mean((XW+b−y)^2)` as a graph. Implement scalar `Value` nodes with `backward()` for add, multiply, exp, and tanh. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Forgetting that a reused node receives gradients from every downstream path; overwrite is wrong—accumulate.
...
For q=(ab+c)^2 at a=2,b=3,c=1, u=6,s=7,q=49 and adjoints are da=42,db=28,dc=14. For x^2+x at x=3, two paths give 6+1=7; this confirms gradients accumulate at reused nodes. For mean((a+b)^2) at a=1,b=2, each input derivative is 6. Shared embeddings in multi-task pricing need all contributions. Debug stale accumulation by resetting gradients per example; a detached metric must never be part of backward.

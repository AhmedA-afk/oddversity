...
order: 221
status: live
summary: "Represent state, uncertainty, and action over time when observations are incomplete and choices influence what happens next."
...
Represent state, uncertainty, and action over time when observations are incomplete and choices influence what happens next. Knowledge-based AI is strongest when it makes its representation and uncertainty explicit: a reader can inspect what the system was told, which inference step it took, and which assumption would change the recommendation. That is a different promise from a system that only produces a score. It is especially valuable where a decision must be reviewed, justified, or revised as new evidence arrives.
...
A belief state assigns probabilities to hidden states; after action `a` and observation `o`, update by transition and observation likelihood, then normalize.
...
A belief state is only as good as the transition and observation models. Do not confuse an internal estimate with verified state.

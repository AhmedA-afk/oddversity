...
status: live
order: 212
summary: "Build detection reasoning from geometry, matching rules, and error taxonomy."
duration: "28 min read"
---

For boxes (A,B), IoU is (\lvert A\cap B\rvert/\lvert A\cup B\rvert).
If two 100x100 boxes overlap on 50x100 pixels, IoU is (5000/(10000+10000-5000)
=1/3). A detector may find the object but fail a 0.5 IoU criterion because its
localization is poor.

Anchors or queries are matched to ground truth; losses combine classification and box
regression. Non-maximum suppression (NMS) retains the highest score then suppresses
same-class boxes over an IoU threshold. Scenario: class-agnostic NMS can erase a
person riding a bicycle. Scenario: crowded cells require softer or set-based matching.
Scenario: small objects need high-resolution features, not merely lower NMS threshold.

```python
keep = []
for b in sorted(boxes, key=lambda b: b.score, reverse=True):
    if all(iou(b, k) < .5 for k in keep): keep.append(b)
```

## Diagnostics

Break false positives into background, duplicate, classification, and localization
errors. Report AP across IoU thresholds (not one threshold only), object sizes, and
classes. Draw predictions and ground truth jointly; a numeric AP cannot reveal a box
coordinate convention mismatch.

## Assignment

Hand-calculate IoU for three box pairs and run NMS on five scored boxes. Implement IoU
tests for non-overlap, containment, and touching edges. Train a toy detector, submit a
size-stratified AP table and 30 visual errors, then recommend a threshold for a safety
camera with an explicit false-alarm budget.


## Advanced studio: calculation, implementation, and decision evidence

The compact explanation above is only a starting point. To master **object detection boxes and iou**, make

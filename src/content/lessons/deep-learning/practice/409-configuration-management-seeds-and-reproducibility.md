...
A run identifier should hash the resolved configuration (c), code revision (r), manifest (m), and environment (e): (id=H(c,r,m,e)). A seed controls a pseudorandom sequence; it does not identify these other inputs. Verify repetition with a tolerance interval, not a visual comparison alone.
...
Two runs share a seed but use a newer tokenizer; their metrics differ for a legitimate, explainable reason.
...
A sweep override changes warmup steps but the printed configuration shows only defaults; the winning run cannot be recovered.
...
A data loader's worker seeds depend on startup order; a deterministic mapping from seed, epoch, and worker fixes it.
...
- **Symptom → likely cause → first check:** A sweep override changes warmup steps but the printed configuration shows only defaults; the winning run cannot be recovered. Inspect the smallest deterministic fixture before changing hyperparameters.
...
A hyperparameter is part of the experiment, not a comment in a notebook. Use a validated configuration with immutable defaults, overrides recorded in the run directory, and derived settings printed at launch. Seed Python, NumPy, framework generators, workers, and deterministic backends while documenting operations that remain nondeterministic.

## Worked scenario

A seed alone does not reproduce a run if the data manifest, preprocessing code, library versions, or mixed-precision setting differ. Hash the resolved configuration and write environment metadata next to outputs.
...
Run the same configuration twice and classify differences as numerical tolerance, data-order variation, or bug. Change exactly one parameter in a third run and demonstrate that the diff is visible in the config comparison.

## Failure modes to diagnose

Do not claim bitwise reproducibility without hardware and kernel evidence. Do not mutate a shared YAML file after the run begins.

## Decision standard

A run directory should let a teammate rerun a result without asking what “the final settings” were.

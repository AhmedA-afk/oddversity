...
track: "deep-learning"
status: live
summary: "Generation is an inference policy over a trained distribution; decoding can change quality, diversity, and risk."
...
Temperature uses $p_T(i)\propto\exp(z_i/T)$. Top-$p$ samples from the smallest set whose mass exceeds $p$; beam search maximizes a length-adjusted sequence score.
...
For logits $[2,1,0]$, lowering $T$ sharpens the highest token; calculate probabilities at $T=1$ and $T=.5$.
...
Greedy decoding can repeat a locally likely phrase; a repetition penalty may help but can also suppress valid repetition.
...
For a customer-support system, constrain tools and citations rather than relying on a creative decoding setting to make facts true.

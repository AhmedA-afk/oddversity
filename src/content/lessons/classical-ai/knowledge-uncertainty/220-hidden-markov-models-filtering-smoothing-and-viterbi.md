...
order: 220
status: live
summary: "Infer an evolving hidden state from noisy observations while separating online belief updates from retrospective explanations."
...
Filtering applies `P(z_t|x_1:t) ∝ P(x_t|z_t) Σ_z P(z_t|z)P(z|x_1:t-1)`. Viterbi instead keeps the most likely state path.
...
HMMs assume limited memory and often simple emissions. Check whether the state abstraction is adequate before tuning probabilities.

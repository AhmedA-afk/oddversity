...
order: 209
status: live
summary: "Combine quantifiers, substitutions, and resolution to derive relational conclusions from a rule base."
...
A rule `∀x (bird(x) → animal(x))` and fact `bird(kiwi)` produce `animal(kiwi)` after standardizing variables and applying the relevant substitution.
...
Skolemization preserves satisfiability for refutation, not the original meaning of existential witnesses in every conversational sense.

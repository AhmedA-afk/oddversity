...
Treat regularisation as a set of explicit inductive biases monitored against validation evidence. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Ridge adds λ||θ||²; early stopping selects a point along the optimisation path; label smoothing replaces one-hot y with (1−ε)y+ε/K, discouraging infinite confidence.
...
1) Derive ridge gradient. 2) Smooth a 3-class target with ε=.1. 3) Select an early-stop epoch from noisy validation losses. 4) Explain why train loss can rise with augmentation.
...
Run a λ sweep with fixed split and seeds; log the chosen checkpoint, confidence histogram, and validation metric—not only final train loss. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Choosing hyperparameters on the test set; early stopping still needs a validation protocol and checkpoint restore.
...
For W=[2,-3], λ=.1, ridge contributes [.4,-.6]. With K=3, ε=.1 and class 1, smoothing target is [.033,.933,.033]. From validation losses [.8,.6,.55,.57,.61], choose and restore epoch 3. Compare low training CE/high test NLL to a less-confident alternative. Moderation systems may value calibrated uncertainty, not just fit. Debug test-set checkpoint selection and report what metric controls early stopping.

...
usable_rows = raw_rows * validity_rate * join_rate * label_coverage
...
1. **Conservative path.** A demand forecast joins point-of-sale, promotions, stock-outs, and calendar data. A late promotion feed can make yesterday's feature look like tomorrow's knowledge. Start with a visible recommendation and a trained human owner. Measure decisions before automating. This reveals whether the proposed model changes useful work or merely produces plausible text.
...
Specify producers, allowed fields, retention, schema checks, late-arrival policy, and a versioned training snapshot.

...
track: "deep-learning"
status: live
summary: "Objectives define prediction tasks; tokenization defines the units whose likelihood and costs you report."
...
Autoregressive LM loss is $-\sum_t\log p(x_t\mid x_{<t})$. Masked objectives hide a subset; span corruption predicts removed spans. They induce different information flows.
...
Perplexity is $\exp(L)$ only when $L$ is average natural-log loss over comparable tokens; it cannot be compared across tokenizers naively.
...
A rare medical term split into many pieces increases its token cost and may degrade exact extraction.
...
For code completion, causal next-token training matches left-to-right use; fill-in-the-middle requires an explicitly trained format.
...
Tokenize raw bytes, Unicode, and whitespace test cases; silent normalization can erase meaningful identifiers.

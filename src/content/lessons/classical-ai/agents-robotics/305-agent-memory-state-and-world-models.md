...
Belief update is p(x_t|z_1:t) proportional to likelihood times predicted belief; attach source, time, confidence, and TTL. Before writing code, declare state variables, observations, permitted actions, objectives, hard constraints, and termination conditions. Preserve uncertainty instead of inventing certainty: a measurement has a source, age, coordinate frame, confidence, and failure mode. A soft objective ranks feasible choices; it must never waive a hard constraint.

**Worked trace.** A service robot remembers room access rules while minimizing retention of visitor details. Suppose action A saves 4 minutes, while action B costs 1 extra minute but keeps the agent in a safer operating region. If A has a 0.03 chance of a loss measured as 500 safety units, its expected risk cost is $0.03 \times 500 = 15$. If time is worth 1 unit per minute, the apparent gain of 4 is dominated by risk. The correct response is B, a new observation, or escalation—not an unqualified “optimal” action. This small calculation is valuable because reviewers can change the inputs and challenge the decision.
...
Separate working, episodic, semantic, and map stores; resolve conflicts by provenance and fresh evidence. Build the following interfaces even in a small prototype:

...
Run tuning as a controlled experiment with budgets, priors, and honest selection. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Random search often beats grids in high-dimensional spaces when few coordinates matter. Optimise an explicitly declared validation metric; reserve the test set for one final estimate. Use log-uniform priors for scale parameters such as learning rate.
...
1) Compare 3×3 grid with 9 random samples. 2) Sample η from [1e−5,1e−2] log-uniformly. 3) Specify a pruning rule. 4) Explain multiple-comparison optimism.
...
Write a search manifest with seed, budget, search space, split, metric, and checkpoint rule. Re-run the top three configs across five seeds. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Tuning on test accuracy or reporting the best lucky seed. A search result without configuration provenance is not evidence.
...
A 3×3 grid spends nine runs in rigid patterns; nine random log-uniform samples can cover more relevant learning-rate scales. A log10 draw -3.4 gives 4e-4. Trial scores .80,.81,.82 with sd .04 are not decisive wins. An ad system’s tiny metric gain may fail cost or latency gates. Debug test-set leakage, premature pruning before warmup, and single-seed finalists; manifest every split hash, budget, metric, and configuration.
...
For **Hyperparameter Search and Experiment Design**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

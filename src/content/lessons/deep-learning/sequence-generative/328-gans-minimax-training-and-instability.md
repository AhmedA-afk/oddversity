...
track: "deep-learning"
status: live
summary: "GANs learn a generator by competing with a discriminator, creating sharp outputs but a delicate game dynamics problem."
...
The original objective is $\min_G\max_D\mathbb E_{x\sim p_{data}}\log D(x)+\mathbb E_{z\sim p(z)}\log(1-D(G(z)))$. Non-saturating generator loss uses $-\log D(G(z))$.
...
At equilibrium the discriminator cannot distinguish distributions and assigns roughly .5, but a .5 score alone does not prove diverse samples.
...
If a discriminator gives fake probability .1, the non-saturating generator loss is $-\log(.9)$ only under a particular convention; verify label definitions.
...
Mode collapse can generate many realistic faces with one pose; evaluate coverage and nearest-neighbor duplicates, not just a gallery.
...
If D wins instantly, alter learning rates, augmentation, regularization, or architecture—but preserve a fixed evaluation protocol.

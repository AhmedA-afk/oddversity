...
track: "deep-learning"
status: live
summary: "Value functions compress expected return; their errors and assumptions determine control quality."
...
The Bellman expectation equation is $V^\pi(s)=\sum_a\pi(a\mid s)\sum_{s',r}P(s',r\mid s,a)[r+\gamma V^\pi(s')]$. Q-values condition additionally on action.
...
For a state with reward 2 then terminal value 5 under $\gamma=.8$, one-step target is $2+.8(5)=6$.
...
Policy iteration alternates evaluating a policy and improving it greedily; value iteration combines both under model assumptions.
...
In inventory control, a state abstraction that omits lead time makes Markov assumptions false and produces seemingly random value errors.
...
Bootstrap targets can propagate a wrong high estimate. Track temporal-difference error by state region and compare to Monte Carlo returns where possible.

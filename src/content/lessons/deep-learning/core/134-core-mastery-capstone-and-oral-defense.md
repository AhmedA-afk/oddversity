...
Demonstrate end-to-end command of the module through a reproducible model investigation and technical defense. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Your report must connect data-generating assumptions, tensor shapes, objective, gradients, optimisation trace, evaluation uncertainty, and deployment decision. Every claim should be backed by an artifact, calculation, test, or cited limitation.
...
1) Select a task and define decision cost. 2) Produce a hand gradient check. 3) Run an ablation. 4) Defend threshold and calibration. 5) Diagnose an injected failure.
...
Submit code, fixed split IDs, run manifests, learning curves, calibration and slice reports, model card, and a 10-minute oral defense. Rubric: correctness 30, evidence 25, reasoning 25, reproducibility 20. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Polished notebooks without replay instructions or negative results. A strong defense explains what failed, why, and what would change under a new constraint.
...
For a triage capstone, hand-compute one BCE gradient and confirm a numerical derivative. Compare baseline and no-augmentation ablation with identical split/seed/budget. Use 10:1 FN:FP cost to defend a threshold and show referral coverage. Inject swapped labels and identify the earliest alarm. Hiring or medicine requires a harm analysis and a justified no-release option. Debug best-checkpoint-only reporting and non-replayable notebooks; distinguish a measured result, causal claim, and assumption in oral defense.
...
For **Core Mastery Capstone and Oral Defense**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

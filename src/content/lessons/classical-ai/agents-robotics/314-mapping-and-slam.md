...
SLAM jointly estimates trajectory X and landmarks M via argmax p(X,M|Z,U); loop closure reduces accumulated error. Before writing code, declare state variables, observations, permitted actions, objectives, hard constraints, and termination conditions. Preserve uncertainty instead of inventing certainty: a measurement has a source, age, coordinate frame, confidence, and failure mode. A soft objective ranks feasible choices; it must never waive a hard constraint.

**Worked trace.** A robot maps a warehouse, returns to a known dock, and discovers that its odometry drifted. Suppose action A saves 4 minutes, while action B costs 1 extra minute but keeps the agent in a safer operating region. If A has a 0.03 chance of a loss measured as 500 safety units, its expected risk cost is $0.03 \times 500 = 15$. If time is worth 1 unit per minute, the apparent gain of 4 is dominated by risk. The correct response is B, a new observation, or escalation—not an unqualified “optimal” action. This small calculation is valuable because reviewers can change the inputs and challenge the decision.
...
Store keyframes, landmarks, pose graph edges, uncertainty, and map version; verify loop candidates before fusion. Build the following interfaces even in a small prototype:

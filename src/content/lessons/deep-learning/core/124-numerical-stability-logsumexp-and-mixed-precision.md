...
1) Compute stable logsumexp of [1000,999]. 2) Compare naive sigmoid at −100. 3) Explain catastrophic cancellation. 4) Diagnose NaN after enabling fp16.
...
Implement stable softmax/CE; assert finite loss and gradients on logits ranging from −1000 to 1000. Add a loss-scale overflow detector in pseudocode. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Adding ε randomly until NaNs vanish; identify the unstable operation and record dtype, range, and first failing batch.
...
For [1000,999], stable logsumexp is 1000+log(1+e^-1)≈1000.313, while naive exp overflows. For z=-100,y=1, stable BCE is near 100 without log(0). Compare fp16 loss of detail when adding .001 and -.001 to fp32 accumulation. A translation service needs accelerator throughput but must capture first nonfinite incident data. Debug by identifying the first unstable operation rather than sprinkling epsilon; use max-subtracted softmax and skip overflowed updates.
...
For **Numerical Stability, LogSumExp, and Mixed Precision**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

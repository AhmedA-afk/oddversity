...
Total value equals task benefit minus operating cost minus risk exposure; metrics must include safety and service quality, not throughput alone. Before writing code, declare state variables, observations, permitted actions, objectives, hard constraints, and termination conditions. Preserve uncertainty instead of inventing certainty: a measurement has a source, age, coordinate frame, confidence, and failure mode. A soft objective ranks feasible choices; it must never waive a hard constraint.

**Worked trace.** Compare a warehouse picker, crop-monitoring drone, assistive robot, and inspection crawler. Suppose action A saves 4 minutes, while action B costs 1 extra minute but keeps the agent in a safer operating region. If A has a 0.03 chance of a loss measured as 500 safety units, its expected risk cost is $0.03 \times 500 = 15$. If time is worth 1 unit per minute, the apparent gain of 4 is dominated by risk. The correct response is B, a new observation, or escalation—not an unqualified “optimal” action. This small calculation is valuable because reviewers can change the inputs and challenge the decision.
...
Create a deployment dossier: operating design domain, hazards, data sources, controls, acceptance tests, and rollback process. Build the following interfaces even in a small prototype:

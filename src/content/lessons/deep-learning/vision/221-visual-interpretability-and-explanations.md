...
status: live
order: 221
summary: "Generate visual explanations as diagnostic evidence, not proof of model reasoning."
duration: "25 min read"
---

Saliency uses (\left|\partial z_c/\partial x\right|) to show local sensitivity.
Grad-CAM weights a feature map by the spatially averaged class gradient, then applies a
positive combination. A heatmap can identify a watermark shortcut, but it does not show
that unhighlighted pixels were irrelevant or that highlighted pixels caused the
decision.

Scenario 1: a classifier highlights hospital tokens, revealing site leakage. Scenario
2: a diffuse map on a true positive may reflect distributed evidence, not failure.
Scenario 3: an adversarially altered image can keep a plausible heatmap while changing
the label. Use perturbation tests: delete/occlude salient regions and measure score
change, while checking that occlusion itself is not OOD.

```python
score = logits[:, target].sum()
score.backward()
saliency = image.grad.abs().amax(dim=1)
```

## Diagnostic protocol

Compare explanations across seeds, augmentations, and similarly predicted images.
Overlay with the image at correct scale; never smooth a map until it looks persuasive.
Test insertion/deletion curves and counterfactual crops. When human experts annotate
evidence, measure agreement before using it as ground truth for attention.

## Assignment

Implement raw saliency and Grad-CAM for one CNN. For 10 correct and 10 incorrect
predictions, create a table of claimed evidence, occlusion result, and root cause.
Construct one synthetic shortcut dataset and demonstrate that a high metric plus a
salient corner can coexist. Write a two-paragraph user warning explaining why the map
must not be presented as a diagnosis or causal explanation.


## Advanced studio: calculation, implementation, and decision evidence

The compact explanation above is only a starting point. To master **visual interpretability and explanations**, make

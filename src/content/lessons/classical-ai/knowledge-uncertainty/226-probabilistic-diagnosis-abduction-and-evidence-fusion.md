...
order: 226
status: live
summary: "Rank explanations for observations while accounting for base rates, test reliability, and dependencies among symptoms."
...
Rank explanations for observations while accounting for base rates, test reliability, and dependencies among symptoms. Knowledge-based AI is strongest when it makes its representation and uncertainty explicit: a reader can inspect what the system was told, which inference step it took, and which assumption would change the recommendation. That is a different promise from a system that only produces a score. It is especially valuable where a decision must be reviewed, justified, or revised as new evidence arrives.
...
Given competing causes, compare posterior odds by multiplying prior odds by likelihood ratios only when the evidence model justifies the factorization.
...
The most probable explanation can still be wrong, and correlated evidence can be double-counted. Provide alternatives and escalation conditions.

...
track: "deep-learning"
status: live
summary: "Flows use invertible transformations to map a simple base density into data while retaining tractable likelihoods."
...
For $x=f(z)$, $\log p_X(x)=\log p_Z(z)-\log|\det \partial f/\partial z|$. Architecture must make inverse and Jacobian determinant efficient.
...
An affine coupling layer transforms part of a vector conditioned on the rest, making its triangular Jacobian easy to evaluate.
...
A flow can assign high likelihood to visually strange images because likelihood reflects density under representation and dimensions; pair it with task-specific diagnostics.
...
Check invertibility numerically by applying inverse(forward(x)); floating-point drift is an implementation bug, not a modelling discovery.

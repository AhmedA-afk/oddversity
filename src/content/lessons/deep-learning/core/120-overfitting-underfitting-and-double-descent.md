...
Read learning curves as evidence about capacity, data, and optimisation rather than as slogans. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Underfitting shows high train and validation error; classic overfitting shows a widening gap. In interpolation regimes, test error can show double descent as parameters pass the point where training error reaches zero.
...
1) Interpret three train/validation plots. 2) Identify interpolation. 3) Explain noisy-label memorisation. 4) Choose data versus capacity as the first intervention.
...
Train a width sweep with fixed data and seeds; plot train error, validation error, and parameter count. Write a causal hypothesis for each regime. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Assuming every U-shaped validation curve is overfitting; changing augmentation, schedules, or split quality can produce similar shapes.
...
For **Overfitting, Underfitting, and Double Descent**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

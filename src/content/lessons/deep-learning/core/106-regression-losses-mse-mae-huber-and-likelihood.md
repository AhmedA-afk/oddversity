...
Choose regression objectives from assumptions about noise and business cost. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

MSE=(1/n)Σ(y−ŷ)² corresponds to fixed-variance Gaussian noise; its derivative w.r.t. ŷ is 2(ŷ−y)/n. MAE targets the conditional median. Huber is quadratic near zero and linear beyond δ.
...
1) Compare MSE/MAE for residuals [1,1,10]. 2) Differentiate a one-point MSE. 3) Evaluate Huber at residual .5 and 3 with δ=1. 4) Decide whether a delayed delivery deserves asymmetric cost.
...
Fit all three losses to data with one outlier; report the fitted slope and residual histogram. Design a pinball-loss extension for the 90th percentile. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Calling MSE robust because its average looks stable; one large residual dominates both its loss and gradient.
...
For residuals [1,1,10], mean squared loss is 34 and mean absolute loss is 4. For r=3, δ=1, Huber is 2.5 with gradient 1, versus MSE gradient 6. For y=[2,4], yhat=[3,3], mean MSE is 1 and output gradients [1,-1]. A late delivery may cost ten times an early one, which argues for an asymmetric quantile loss rather than a familiar default. Debug transformed targets: model selection on log-space MSE must still report business error after inverse transformation.
...
For **Regression Losses: MSE, MAE, Huber, and Likelihood**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

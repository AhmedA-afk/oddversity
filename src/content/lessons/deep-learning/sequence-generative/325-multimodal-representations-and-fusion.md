...
track: "deep-learning"
status: live
summary: "Multimodal systems require aligned representations, modality-specific failure analysis, and defensible fusion choices."
...
Late fusion combines scores; early fusion mixes tokens/features. A contrastive image-text objective raises paired similarity against mismatched pairs.
...
For a batch of paired image/text embeddings, use the diagonal as positives and off-diagonal pairs as negatives; inspect duplicate or semantically equivalent pairs.
...
A radiology image plus report may improve triage, but a missing report at deployment changes the input distribution and must be simulated.
...
In product search, OCR noise can make text dominate vision; measure modality ablation and corrupted-modality performance.
...
Sensitive attributes can be visible in pixels or language. Access, consent, and downstream use must be audited at the system level.

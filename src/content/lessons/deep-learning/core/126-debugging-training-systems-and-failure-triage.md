...
Debug models with ordered tests that isolate data, objective, gradients, and loop mechanics. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

A reliable escalation: overfit 10 examples; verify labels and split; inspect input/activation/gradient ranges; finite-difference one batch; then scale up. An inability to overfit a tiny clean set is a system bug until proven otherwise.
...
1) Training loss flat. 2) Loss becomes NaN at step 20. 3) Validation exceeds training. 4) Accuracy stuck at class prevalence.
...
Create a debug checklist and instrument loss, LR, parameter norm, gradient norm, activation percentiles, class counts, and sample IDs. Write one hypothesis per signal. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Changing five knobs at once. Preserve a minimal failing run and make one intervention per experiment.
...
Ten-class loss stuck at 2.303 suggests unchanged logits or labels; inspect both. A NaN at step 20 requires comparing ranges and sample IDs at steps 19/20. Validation above train can result from training augmentation/dropout. Prevalence-level accuracy needs sampling and threshold checks. A sensor firmware change can create all-zero fields, visible in percentiles. Debug one controlled variable at a time; preserve a minimal failing run and prove the model overfits a tiny clean fixture.
...
For **Debugging Training Systems and Failure Triage**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

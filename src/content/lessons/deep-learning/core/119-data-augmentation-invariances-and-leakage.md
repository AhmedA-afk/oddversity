...
Encode only defensible invariances, and treat augmentation as part of the training data pipeline. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Augmentation samples x′∼T(x) while keeping y when the label is invariant. It changes the training distribution, so validation/test must remain untouched unless test-time augmentation is explicitly averaged.
...
1) Flip a cat image versus a digit 6. 2) Add noise to tabular income. 3) Time-shift a forecast target and expose leakage. 4) Mix two labels and compute soft targets.
...
Specify transformations and parameter ranges for one vision, one audio, and one tabular problem. Run an ablation: baseline, each transform, combined. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Augmenting after splitting but before identity grouping; near-duplicates can cross splits and inflate validation.
...
A two-pixel crop shift may preserve a cat, but 180° rotation can convert 6 into 9. For y_(t+1), any feature from t+1 is leakage even if introduced by a transform. Mixup with λ=.7 produces .7xA+.3xB and soft label [.7,.3]. In inspection, brightness jitter may be valid but rotation may be impossible. Debug group-aware splits before augmentation, preserve transform parameters for failed samples, and keep validation preprocessing deterministic.

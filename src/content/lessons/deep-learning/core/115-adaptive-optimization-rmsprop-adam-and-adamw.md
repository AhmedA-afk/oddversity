...
Derive adaptive step scaling, understand bias correction, and separate AdamW’s weight decay from L2 coupling. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Adam uses m_t=β₁m+ (1−β₁)g and v_t=β₂v+(1−β₂)g²; m̂=m/(1−β₁^t), v̂=v/(1−β₂^t), θ←θ−ηm̂/(√v̂+ε). AdamW decouples θ←(1−ηλ)θ from the adaptive gradient step.
...
1) Calculate Adam’s first step. 2) Compare sparse and dense coordinates. 3) Show bias correction at t=1. 4) Contrast L2 loss term with decoupled decay.
...
Implement AdamW from arrays; test first three updates against hand values and compare train/validation curves with SGD+momentum. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Putting weight decay on bias and normalisation scale by default, or assuming Adam needs no learning-rate schedule.
...
For g=2, β1=.9, β2=.999, first-step m=.2,v=.004, corrected mhat=2,vhat=4, giving unit normalized direction. For g=[1,100], adaptive scaling makes first step magnitudes comparable. With η=.001,λ=.1, AdamW multiplies weights by .9999 before gradient motion. Fine-tuning must usually exempt biases and norm scales from decay. Debug accidental double regularization when L2 is also added to the loss and confirm bias correction on steps 1–3.
...
For **Adaptive Optimisation: RMSProp, Adam, and AdamW**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

...
track: "deep-learning"
status: live
summary: "A fast retriever buys recall; a reranker spends more compute to distinguish fine-grained relevance among candidates."
...
A cross-encoder scores $s(q,d)$ jointly. Pairwise logistic loss is $\log(1+e^{-(s^+-s^-)})$; listwise objectives target ranking distributions.
...
For scores positive 2 and negative 1, pairwise loss is $\log(1+e^{-1})\approx.313$; improving the score gap reduces loss.
...
A lexical retriever finds a policy clause; the reranker should prefer the clause that answers the question rather than one sharing more words.
...
If all candidate documents are irrelevant, reranking cannot recover the answer: log candidate recall separately from final ranking quality.
...
Evaluate MRR, nDCG, and recall with judged queries, and stratify by fresh, rare, and permission-restricted content.

...
track: "deep-learning"
status: live
summary: "Autoencoders learn to reconstruct inputs through a constrained code; reconstruction quality is not a guarantee of useful semantics."
...
An encoder $z=f_\phi(x)$ and decoder $\hat x=g_\theta(z)$ minimize $\|x-\hat x\|^2$ or a likelihood-appropriate loss. A bottleneck or regularizer prevents trivial copying.
...
For binary pixels, Bernoulli negative log-likelihood is more aligned than MSE; choose the observation model before interpreting loss.
...
A 2D latent can reconstruct a smooth handwritten digit manifold but may merge rare symbols; inspect per-class reconstruction error.
...
Denoising autoencoders reconstruct clean x from corrupted input, learning robust local structure rather than exact identity.
...
Anomaly score based on reconstruction can fail when an over-capacity decoder also reconstructs anomalies; validate on true anomalies.

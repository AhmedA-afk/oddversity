...
order: 205
status: live
summary: "Turn a finite logical problem into clauses while preserving the solutions that matter."
...
The implication `A → B` becomes `¬A ∨ B`. For a scheduling rule “at least one of A or B runs,” use `A ∨ B`; for “not both,” add `¬A ∨ ¬B`.
...
Clause conversion is mechanical, but the choice of variables is a modelling decision that can hide crucial distinctions.

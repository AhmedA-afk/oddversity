...
Understand the perceptron as a geometric classifier and know precisely when it must fail. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

A classifier predicts sign(wᵀx+b). Its boundary is wᵀx+b=0 and its signed geometric distance is (wᵀx+b)/||w||. The update on a mistake is w←w+ηyx, b←b+ηy.
...
1) Separate (2,1),(3,1) from (−1,0),(−2,−1). 2) Compute a point’s signed distance for w=(2,1). 3) Run one update on a misclassified point. 4) Explain XOR’s opposing corners.
...
Implement the update loop without libraries; plot errors per epoch for separable data and XOR. State the stopping condition and what non-convergence means. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Confusing score magnitude with probability; multiplying w and b changes scores but not the boundary. A perceptron cannot repair nonlinearly separable features.
...
With w=(1,-1), b=0, x=(2,3), y=+1, the score is -1; η=.5 gives w+=(2,-2), b+=.5. For x=(3,1), the signed distance is 4.5/sqrt(8)=1.59. Scale w,b by two and confirm the boundary remains unchanged while functional score doubles. Finally add the four XOR inequalities to see why no line works. A screening rule needs a calibrated probability after this geometry, not score magnitude. Debug label encoding: the ±1 update is invalid for literal 0/1 labels.
...
For **Perceptrons, Linear Separability, and Geometric Margins**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

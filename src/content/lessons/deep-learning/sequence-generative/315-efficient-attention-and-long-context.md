...
track: "deep-learning"
status: live
summary: "Quadratic attention is a real systems constraint; approximations change what dependencies a model can represent."
...
Dense attention has $O(T^2d)$ score work and $O(T^2)$ weight storage. Sliding windows, blocks, low-rank methods, and kernels trade exactness for scale.
...
At $T=32{,}000$, one head has about $10^9$ pair scores; calculate memory before promising a context window.
...
Local attention handles nearby syntax efficiently but can miss a distant contract clause; add global tokens or retrieval deliberately.
...
Chunking a document without overlap can split evidence across boundaries, making a retrieval failure look like a model reasoning failure.

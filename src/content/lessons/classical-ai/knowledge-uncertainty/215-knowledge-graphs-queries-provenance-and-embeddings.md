...
order: 215
status: live
summary: "Model connected facts with source-aware edges so queries and explanations can travel through a real evidence trail."
...
For a graph edge `(supplier, supplies, component)`, a query can follow paths to find components affected by a supplier outage. Each edge needs a source and validity interval.
...
Graph embeddings can rank plausible links, but they do not turn a missing or stale source into a fact. Keep predictions separate from asserted edges.

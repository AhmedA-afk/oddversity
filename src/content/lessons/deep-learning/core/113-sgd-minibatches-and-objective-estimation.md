...
1) Compare full, batch-1, and batch-32 gradient estimates. 2) Calculate updates for η=.1. 3) Explain shuffling. 4) Diagnose a batch with one rare positive.
...
Implement an epoch loop that logs mean loss, gradient norm, and examples seen. Demonstrate what breaks when the last batch is not handled. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Reading a noisy batch loss as a validation curve; training metrics must be aggregated across comparable data.
...
For N=50,000, B=32 yields 1,563 updates per epoch whereas B=512 yields 98. Example gradients [1,3,5,7] average to 4, but batch [1,3] estimates 2: noise is expected. Four B=8 microbatches reproduce B=32 only when their losses are correctly scaled before one step. Fraud batches may contain rare positives unevenly, so log class counts and examples seen. Debug summed versus mean reductions, which silently change effective learning rate with B.
...
For **SGD, Mini-batches, and Objective Estimation**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

...
Use differentials and trace identities to derive gradients without coordinate-index chaos. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

If L=||XW−Y||_F², dL=2tr((XW−Y)ᵀX dW), so ∇_WL=2Xᵀ(XW−Y). For L=aᵀWb, ∇_WL=abᵀ.
...
1) Derive linear least squares. 2) Differentiate aᵀWb. 3) Derive a bias row-vector gradient. 4) Explain why Frobenius inner products make shapes obvious.
...
Derive and code gradients for ridge regression and for a batched affine layer; compare with autodiff on random matrices. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Applying scalar derivative mnemonics to matrices while ignoring orientation. Differentials preserve dimensions and prevent this.
...
For L=||XW-Y||_F^2, the differential gives dW=2X^T(XW-Y). With X=I, W=[2,3]^T, Y=[1,1]^T, gradient is [2,4]^T. For a=[1,2],b=[3,4], grad of a^TWb is [[3,4],[6,8]]. Add ridge λ=.1,W=[2,3] to obtain additional [.4,.6]. Batched forecasting relies on this shape-correct form. Debug by asserting dW has exactly W’s shape and deriving transposes with traces, not visual guesswork.

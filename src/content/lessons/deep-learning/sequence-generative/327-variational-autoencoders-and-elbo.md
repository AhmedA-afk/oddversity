...
track: "deep-learning"
status: live
summary: "VAEs learn a probabilistic latent variable model while making approximate inference differentiable."
...
The ELBO is $\mathbb E_{q_\phi(z\mid x)}[\log p_\theta(x\mid z)]-KL(q_\phi(z\mid x)\|p(z))\le\log p_\theta(x)$. It balances reconstruction and a prior-shaped latent space.
...
For diagonal Gaussian $q=\mathcal N(\mu,\sigma^2)$ and standard normal prior, $KL=\tfrac12\sum(\mu^2+\sigma^2-\log\sigma^2-1)$.
...
With $\mu=1,\sigma=1$, one latent dimension contributes $.5$ KL; calculate how reducing sigma changes the penalty.
...
Reparameterize $z=\mu+\sigma\odot\epsilon$, $\epsilon\sim\mathcal N(0,I)$, so gradients reach encoder parameters.
...
Posterior collapse occurs when the decoder ignores z; monitor KL by dimension and test whether changing z changes outputs.

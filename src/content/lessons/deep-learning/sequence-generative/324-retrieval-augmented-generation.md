...
track: "deep-learning"
status: live
summary: "RAG joins retrieval and generation, but grounded output requires evidence selection, citation checks, and abstention behavior."
...
A conditional answer is $p(y\mid q)\approx\sum_{d\in C(q)}p(y\mid q,d)p(d\mid q)$. The candidate set is an approximation whose omissions matter.
...
Decompose failure probability into retrieval miss, wrong-rank, context truncation, and unsupported generation; each has a different remedy.
...
For a benefits question, retrieve the current policy version, quote its effective date, and abstain if the evidence is missing.
...
Adding ten weak chunks can drown out one relevant paragraph; evaluate context precision as well as recall.
...
Never treat model-generated citations as evidence. Verify citation spans against retrieved source text before display.

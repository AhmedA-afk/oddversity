...
Turn model scores into decisions with suitable metrics, calibrated probabilities, and abstention logic. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Calibration asks P(Y=1|p≈q)≈q. Brier score is mean(p−y)². A threshold minimises expected cost only after costs, prevalence, and calibration are considered; uncertainty can trigger review rather than forced prediction.
...
1) Compute precision/recall from a confusion matrix. 2) Compute Brier score for three cases. 3) Choose a threshold with FN cost 10×FP. 4) Interpret reliability bins.
...
Evaluate a classifier with ROC/PR, reliability diagram, Brier/NLL, and a cost curve. Add an abstain region and quantify coverage versus risk. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Reporting AUROC as deployment success; it neither sets a threshold nor guarantees calibration or acceptable harm.
...
For p=[.9,.6,.2], y=[1,0,0], Brier is (.01+.36+.04)/3=.137. With FN cost 10 and FP cost 1, calibrated threshold is 1/11≈.091 under the stated action model. From TP=8,FP=2,FN=4, precision=.8 and recall=.667. A sepsis system may abstain/refer uncertain cases; report coverage and residual risk. Debug AUROC-only reporting, calibration on test data, and prevalence drift that invalidates a threshold.
...
For **Evaluation, Thresholds, Calibration, and Uncertainty**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

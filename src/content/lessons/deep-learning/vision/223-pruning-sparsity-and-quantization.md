...
status: live
order: 223
summary: "Reduce model size and latency with hardware-aware pruning and quantization validation."
duration: "27 min read"
---

Uniform affine quantization approximates (x) by
(\hat{x}=s(\operatorname{clip}(\operatorname{round}(x/s)+z,q_{min},q_{max})-z)).
For symmetric int8 with maximum magnitude 1.27, choose (s=1.27/127=0.01);
0.43 becomes round(43)=43 and reconstructs exactly, while 0.435 rounds to 44 or 0.44.
Quantization error is data-dependent and can be catastrophic for outlier channels.

Unstructured pruning removes individual weights but may not speed dense hardware.
Structured channel pruning changes tensor shapes and can yield real latency gains.
Scenario: int8 improves CPU serving but unsupported operations force float fallback.
Scenario: calibration data misses night images, so activation ranges clip at launch.
Scenario: pruning a minority-class feature reduces fairness despite stable average AP.
...
Quantize a vector by hand and calculate absolute/relative error. Compare post-training
quantization and quantization-aware training on day/night and common/rare slices.
Prune channels by a stated criterion, then benchmark *actual* runtime at batch 1 and
batch 32. Submit an operator coverage report, a rollback threshold, and a test that
detects unintended float fallback.


## Advanced studio: calculation, implementation, and decision evidence

The compact explanation above is only a starting point. To master **pruning sparsity and quantization**, make

...
For a routing system, evaluate both model accuracy and workflow utility: correct routing, safe escalation, resolution time, and review burden. Feedback labels (z) are observed only when users or agents act, so (P(z|x)) can differ from the full served population.
...
A dialect-specific request is confidently routed to a generic queue and receives a delayed response.
...
Agent edits improve a summary but reflect time pressure, not necessarily the desired output standard.
...
A policy update changes escalation rules; historical feedback is no longer directly comparable.
...
- **Symptom → likely cause → first check:** Agent edits improve a summary but reflect time pressure, not necessarily the desired output standard. Inspect the smallest deterministic fixture before changing hyperparameters.
...
A support-routing or summarization model sits inside a workflow with customer privacy, multilingual coverage, escalation, knowledge freshness, and feedback bias. Offline language metrics are insufficient unless they connect to resolution quality and safe handoff.

## Worked scenario

Use chronological splits by ticket date, redact sensitive fields, track language and issue-type coverage, and evaluate routing accuracy plus escalation correctness. Treat agent edits as feedback with selection bias: only some outputs are reviewed.
...
Analyze three errors: a confident wrong routing decision, a low-confidence correct summary, and a dialect-specific failure. Specify abstention, human review, logging limits, and retraining eligibility for each.

## Failure modes to diagnose

Using customer text as unrestricted training data violates purpose limitation. Optimizing for handle-time alone can suppress necessary escalation or reduce answer quality.

## Decision standard

Deliver a model card excerpt, review policy, feedback data contract, and post-deployment evaluation plan.

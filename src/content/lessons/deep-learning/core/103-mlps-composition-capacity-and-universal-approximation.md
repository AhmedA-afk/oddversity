...
Turn a perceptron into a multilayer function and connect width/depth to useful—not mystical—capacity. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

An L-layer MLP composes h₀=x, z_l=W_lh_{l−1}+b_l, h_l=φ_l(z_l). Universal approximation is an existence result on compact sets, not a promise of easy training or good generalisation.
...
1) Express XOR with two hidden threshold units. 2) Trace shapes for 10→64→32→3. 3) Compare one wide ReLU layer to two narrow layers on a piecewise-linear function. 4) Count parameters including biases.
...
Write shape-aware pseudocode for `forward(params, x)`. Fit XOR, then deliberately shrink width until it underfits; record the decision regions. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Treating universal approximation as sample efficiency; capacity without data, regularisation, and optimisation can memorise noise.
...
For a 4→5→3 MLP, P=(4+1)5+(5+1)3=43 parameters. A 2→2→1 ReLU construction with hidden units ReLU(x1+x2) and ReLU(x1-x2) evaluates to 3 at (2,1) and 0 at (-2,1). Build XOR with two hidden detectors and evaluate all four corners. Capacity should be chosen after data/safety constraints: a defect camera with small labelled data can memorise backgrounds. Debug output semantics: a regression head must not silently use ReLU if negative values are valid.
...
For **MLPs: Composition, Capacity, and Universal Approximation**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

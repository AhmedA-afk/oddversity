...
system_reliability = product(stage_reliability_i) when failures are independent; weakest_stage bounds the chain
...
Draw the trust boundary, data flow, state store, tool permissions, evaluation gates, observability fields, and failure containment plan.

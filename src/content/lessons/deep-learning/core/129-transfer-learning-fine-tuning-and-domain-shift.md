...
Adapt pretrained representations with a careful protocol for limited data and shifted domains. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

Transfer starts from θ_pre then optimises on target data. Freeze a backbone to reduce variance, then unfreeze gradually with discriminative learning rates; evaluate domain shift explicitly rather than assuming source features transfer.
...
1) Compare linear probe versus full fine-tune. 2) Set 10× smaller backbone LR. 3) Diagnose catastrophic forgetting. 4) Compare in-domain and shifted validation.
...
Fine-tune a pretrained encoder on a small target split; run frozen, last-block, and full variants with identical heads and seeds. Log trainable parameter counts. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Using target test data for adaptation, or assuming an ImageNet-style preprocessing recipe is valid for a new sensor/domain.
...
A 100M backbone plus 1M head means a linear probe trains roughly 1% of parameters; last-block and full tuning alter that risk. Use head LR 1e-3 and backbone 1e-4 as an explicit cautious policy. Scores source .92,target .71,target-tuned .78 quantify the shift. A satellite sensor change alters spectral/label assumptions. Debug target-test adaptation, BN statistic drift, and incomparable frozen/partial/full runs; preserve preprocessing and parameter-group policy in artifacts.

...
track: "deep-learning"
status: live
summary: "Masks define which information is legally visible; an incorrect mask can create perfect-looking leakage."
...
Causal masking sets $M_{ij}=-\infty$ for $j>i$, then $\operatorname{softmax}((QK^\top/\sqrt d)+M)$. Padding masks exclude nonexistent keys.
...
For length 4, row 3 of a causal mask may attend indices 1–3 but never 4; draw this matrix before coding.
...
A next-token model that sees its target through a missing causal mask reports unrealistically low loss.
...
In prefix language modeling, prefix tokens can mutually attend while generated suffix tokens remain causal; this is a different mask, not a flag name.
...
Use finite large negatives carefully in low precision: confirm masked probabilities are exactly or effectively zero.

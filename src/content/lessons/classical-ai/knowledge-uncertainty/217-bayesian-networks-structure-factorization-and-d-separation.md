...
order: 217
status: live
summary: "Use a directed graph to state conditional dependencies compactly and test when observing one variable should change beliefs about another."
...
Use a directed graph to state conditional dependencies compactly and test when observing one variable should change beliefs about another. Knowledge-based AI is strongest when it makes its representation and uncertainty explicit: a reader can inspect what the system was told, which inference step it took, and which assumption would change the recommendation. That is a different promise from a system that only produces a score. It is especially valuable where a decision must be reviewed, justified, or revised as new evidence arrives.
...
For `Rain → WetGrass ← Sprinkler`, the joint factorizes as `P(Rain)P(Sprinkler)P(WetGrass|Rain,Sprinkler)`. Conditioning on WetGrass can make Rain and Sprinkler dependent.
...
A graph is an assumption about dependencies, not a discovered causal truth merely because it fits the data.

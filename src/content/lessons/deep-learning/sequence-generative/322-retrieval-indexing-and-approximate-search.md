...
track: "deep-learning"
status: live
summary: "Retrieval converts a query into a candidate set. Index construction, freshness, filters, and evaluation are part of the model."
...
Exact search ranks all documents by $q^\top d$. Approximate nearest neighbor structures trade recall for latency and memory; measure recall@K against exact search.
...
Recall@K is $1$ if a relevant item appears among the first K; for multiple relevant items use the fraction recovered and report the relevance labels.
...
With 1M documents, metadata filtering after vector search can return no valid result; incorporate tenant and permission filters into candidate generation.
...
A document update must invalidate or version its embedding; stale embeddings can surface revoked or contradictory policy text.
...
Chunk a document by semantic and structural boundaries, then preserve source, section, and access-control metadata.

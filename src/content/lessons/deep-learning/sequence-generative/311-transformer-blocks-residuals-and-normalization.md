...
track: "deep-learning"
status: live
summary: "A transformer alternates contextual mixing with per-token nonlinear transformation, protected by residual paths."
...
A pre-norm block is $x'=x+\operatorname{Attn}(\operatorname{LN}(x))$ and $x''=x'+\operatorname{MLP}(\operatorname{LN}(x'))$. The residual supplies an identity gradient route.
...
The MLP often computes $W_2\phi(W_1x+b_1)+b_2$; it mixes features within a position after attention mixes positions.
...
For a two-token input, attention can exchange information, while the MLP can turn a conjunction of features into a new feature.
...
If removing residuals makes a deep model fail, the issue is path geometry, not proof that attention is useless.
...
Post-norm and pre-norm can train differently at depth; state the architecture exactly when reproducing a result.

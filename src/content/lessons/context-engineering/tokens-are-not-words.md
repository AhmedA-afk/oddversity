...
**A long URL** like `https://api.fernway.example.com/v2/accounts/9f21c-a88e/invoices?status=paid&limit=50` — mostly random-looking path segments, query parameters, and punctuation the vocabulary has rarely seen as a unit. This tiles into far more, far smaller pieces than its character count would suggest under the 4-characters-per-token rule: individual path segments, punctuation, and parameter names each tend to cost their own token or two, so a ~90-character URL can easily run well past 20 tokens instead of the ~22 the rule of thumb would estimate — and often noticeably more once query strings and hashes are involved.

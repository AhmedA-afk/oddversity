...
order: 202
status: live
summary: "A rigorous, practice-led lesson on annotation protocols, disagreement, label error, and construct validity for building AI systems that can be evaluated, challenged, and operated responsibly."
duration: "50–70 min"
updated: "2026-08-30"
---

# Labels, Annotation, and the Limits of Ground Truth
...
available when a decision is made. This lesson examines **annotation protocols, disagreement, label error, and construct validity**. The goal is not to
...
metric misleading here?** In this unit, the relevant mechanism is annotation protocols, disagreement, label error, and construct validity. Treat it
...
For labels, annotation, and the limits of ground truth, use the same discipline with the relevant signals:
inter-annotator agreement, adjudication rate, missing-label rate, and slice-specific error. Estimate uncertainty rather than treating one run as final. For a proportion,
...
Consider a moderation dataset where annotators disagree about sarcasm, abuse, and quoted speech. Begin with a narrow workflow map: who supplies an input, what
...
The characteristic failure to avoid is **treating a convenient label as the construct that actually matters**. It often survives a dashboard
...
4. Compute inter-annotator agreement, adjudication rate, missing-label rate, and slice-specific error; publish slice counts and uncertainty.
...
## Dataset design: sampling frame before annotation

Labels cannot repair a dataset whose sampling frame does not support the deployment
claim. Before assigning tasks to raters, document the target population, inclusion and
exclusion rules, time window, source channels, collection incentives, and expected
subgroups. For instance, a support dataset containing only conversations that reached a
human agent is a selected sample: it systematically misses users who abandoned the
interface, were blocked by accessibility barriers, or solved the issue elsewhere.

Make coverage measurable. Compare the planned deployment mix with the observed data
mix by language, device, geography, time, and outcome where appropriate. A large
dataset can still be unrepresentative; a balanced table can still exclude a population
that matters. The dataset card should therefore state both the intended coverage and
known gaps. When a gap is material, collect more data, narrow the model claim, or add
a defer-to-human pathway. Do not declare the labels “ground truth” when the sampling
process cannot justify that confidence.

...
order: 210
status: live
summary: "Use data-driven rule firing when new facts should propagate through a known dependency network."
...
If `fever(lee)` and `cough(lee)` are facts, the rule `fever(x) ∧ cough(x) → review(x)` adds `review(lee)`. Continue until no new facts appear.
...
Forward chaining can derive irrelevant facts. Index rules by their premises and track the source of every conclusion.

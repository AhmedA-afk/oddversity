...
total_cost = build_cost + requests * unit_cost + error_cost * error_rate
...
1. **Conservative path.** Invoice validation: a mandatory tax identifier is best enforced by a rule; a fraud score can use tabular ML; damaged-document detection may require vision; an unstructured vendor email may justify an LLM. Start with a visible recommendation and a trained human owner. Measure decisions before automating. This reveals whether the proposed model changes useful work or merely produces plausible text.
...
Create a decision table with a rule baseline, a statistical baseline, and a foundation-model option; reject options that cannot be evaluated safely.

...
Reason about dropout’s noise model and correctly use inverted scaling. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

With keep probability q, inverted dropout uses h̃=m⊙h/q, m∼Bernoulli(q), so E[h̃]=h during training and no scale adjustment is needed at evaluation.
...
1) Compute expected activation for q=.8. 2) Enumerate masks for a two-unit layer. 3) Contrast dropout with deterministic weight decay. 4) Diagnose a model whose evaluation still drops units.
...
Implement dropout with a seeded RNG; verify mean preservation across 10,000 masks and that eval outputs are deterministic. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Applying dropout to output probabilities or forgetting eval mode; both corrupt calibration and reproducibility.
...
For h=[2,4], keep q=.5 and mask [1,0], inverted dropout gives [4,0], whose expectation across masks remains [2,4]. With q=.8, retained values multiply by 1.25. Compare independent per-example masks to an accidental shared scalar mask. Identical recommender requests must be deterministic in ordinary serving; MC dropout is an explicit exception. Debug whether an API parameter means drop rate or keep rate, and unit-test train expectation and eval determinism separately.
...
For **Dropout, Ensembles, and Train/Eval Semantics**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

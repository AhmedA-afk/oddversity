...
track: "deep-learning"
status: live
summary: "BPTT assigns credit through an unrolled recurrent computation graph."
...
Let $h_t=f(a_t)$, $a_t=W_hh_{t-1}+W_xx_t$, and $L=\sum_t\ell_t$. The recurrence is $\delta_t=\partial\ell_t/\partial a_t+W_h^\top\delta_{t+1}\odot f'(a_t)$.
...
The parameter gradient shares weights across time: $\partial L/\partial W_h=\sum_t\delta_th_{t-1}^\top$. Accumulation is why a long sequence can dominate an update.
...
For scalar $h_t=wh_{t-1}$ and loss $h_3$, $\partial h_3/\partial w=3w^2h_0$; compute it by hand before trusting autograd.
...
In character prediction, an error on the final character can update embeddings from many previous positions through the recurrence.
...
Truncating BPTT to 128 tokens saves memory but makes dependencies beyond that window unable to receive direct gradient credit.

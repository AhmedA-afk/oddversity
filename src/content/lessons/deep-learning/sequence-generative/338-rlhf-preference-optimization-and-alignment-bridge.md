...
track: "deep-learning"
status: live
summary: "RLHF turns human comparisons into a training signal, but preferences are noisy, contextual, and not a complete alignment specification."
...
A reward model can fit $P(y^+\succ y^-)=\sigma(r_\phi(x,y^+)-r_\phi(x,y^-))$. Policy optimization then balances reward against divergence from a reference policy.
...
Annotators may prefer confident wrong answers over cautious correct ones; include factuality and abstention-specific evaluation sets.
...
Reward hacking can produce a response optimized for the judge rather than the user. Audit with held-out human and objective checks.
...
DPO-like direct preference objectives avoid an explicit RL rollout loop but do not eliminate data bias, reward misspecification, or deployment feedback.

...
Test what a representation exposes while avoiding claims a probe cannot support. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

A linear probe fits a simple classifier to frozen features. High probe accuracy shows information is linearly accessible under that setup; it does not prove the original model used that information causally.
...
1) Probe class labels at two layers. 2) Shuffle labels as a sanity check. 3) Compare random features. 4) Build a counterfactual slice where shortcut cues fail.
...
Extract frozen embeddings, train regularised linear probes across layers, and report confidence intervals, control probes, and worst-group performance. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Treating probe accuracy as an explanation. Include random-feature, label-permutation, and dataset-shift controls.
...
Layer probes scoring [.65,.78,.82] can show increasing accessible signal; a ten-class shuffled-label control should be near .10. Compare learned .82 with random-feature .55. If an overall .82 becomes .51 on a shortcut-controlled slice, the representation likely carries spurious cues. A safety representation may expose sensitive information as a governance concern. Debug test-tuned probes and inconsistent feature scaling; include permutation, random-feature, and shifted-data controls.
...
For **Representation Probing and Diagnostic Evaluation**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

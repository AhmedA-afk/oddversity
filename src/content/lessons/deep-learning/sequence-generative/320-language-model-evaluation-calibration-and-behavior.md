...
track: "deep-learning"
status: live
summary: "A useful language model is judged by task behavior, calibration, robustness, and harm—not one loss number."
...
Perplexity measures average next-token surprise, whereas task success may require factual support, exact formatting, abstention, or safe action selection.
...
Expected calibration error bins confidence: $ECE=\sum_b\frac{|B_b|}{n}|\operatorname{acc}(B_b)-\operatorname{conf}(B_b)|$. Audit bins and sample sizes.
...
A model with lower perplexity can still choose an unsupported answer in question answering; score attribution separately.
...
For extraction, exact-match and span-level F1 catch different errors; report both with normalized and raw text policies.
...
A benchmark answer appearing in training invalidates the apparent gain; retain provenance and contamination probes.

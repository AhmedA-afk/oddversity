...
Select activations by gradient and output semantics, then reason about dead or saturated units. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

ReLU(z)=max(0,z); its derivative is 1 for z>0. Sigmoid′=σ(z)(1−σ(z))≤1/4, explaining saturation. GELU approximately gates inputs smoothly; softmax belongs to a vector output, not hidden scalar units.
...
1) Compute ReLU gradients at −2,0,3. 2) Compare sigmoid gradients at 0 and ±8. 3) Apply softmax to [2,1,−1]. 4) Identify a dying ReLU batch.
...
Build a small classifier with ReLU, leaky ReLU, and GELU; log activation mean, variance, and zero fraction after each epoch. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Using sigmoid hidden layers at depth without addressing saturation; applying softmax before a logits-based cross-entropy routine.
...
At z=-2,0,3, ReLU outputs 0,0,3 while sigmoid is about .119,.5,.953 with derivatives .105,.25,.045. At z=8 sigmoid derivative is about .000335, demonstrating saturation. For logits [2,1,-1], stable softmax is roughly [.705,.259,.035]. A triage system can use sigmoid only at a binary output; hidden sigmoid saturation is a training risk. Debug dead units by tracking layerwise zero fraction and activation percentiles, not only loss.
...
For **Activation Functions and Signal Propagation**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

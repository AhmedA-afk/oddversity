...
order: 220
status: live
summary: "A rigorous, practice-led lesson on authority allocation, automation bias, workload, contestability, and override design for building AI systems that can be evaluated, challenged, and operated responsibly."
duration: "50–70 min"
updated: "2026-08-30"
---
...
available when a decision is made. This lesson examines **authority allocation, automation bias, workload, contestability, and override design**. The goal is not to
...
metric misleading here?** In this unit, the relevant mechanism is authority allocation, automation bias, workload, contestability, and override design. Treat it
...
For human-in-the-loop and meaningful oversight, use the same discipline with the relevant signals:
override rate, agreement, time burden, appeal outcomes, automation-bias probes, and user trust. Estimate uncertainty rather than treating one run as final. For a proportion,
...
Consider a clinician sees a risk score but has 30 seconds and no explanation of uncertainty. Begin with a narrow workflow map: who supplies an input, what
...
The characteristic failure to avoid is **adding a human click at the end of a pipeline and calling that meaningful oversight**. It often survives a dashboard
...
4. Compute override rate, agreement, time burden, appeal outcomes, automation-bias probes, and user trust; publish slice counts and uncertainty.

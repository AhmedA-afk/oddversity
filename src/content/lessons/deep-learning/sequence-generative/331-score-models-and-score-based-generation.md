...
track: "deep-learning"
status: live
summary: "Score models estimate the gradient of log density, enabling generation by reversing a noising process."
...
The score is $s_\theta(x,t)\approx\nabla_x\log p_t(x)$. Denoising score matching relates it to predicting perturbation noise under known Gaussian corruption.
...
For Gaussian corruption with variance $\sigma^2$, the conditional score is $-(x_t-x_0)/\sigma^2$; calculate its direction for a one-dimensional perturbation.
...
Langevin update $x\leftarrow x+\eta s_\theta(x)+\sqrt{2\eta}\xi$ moves toward high density while injecting noise.
...
A score estimate near a training point can still be unreliable in low-density regions; examine samples across the intended condition space.
...
Continuous-time notation does not remove numerical error: validate solver steps, seeds, and stopping criteria.

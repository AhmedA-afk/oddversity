...
track: "deep-learning"
status: live
summary: "Reinforcement learning optimizes long-run consequences under uncertainty; define the environment before choosing an algorithm."
...
An MDP has states $s$, actions $a$, transitions $P$, reward $r$, and discount $\gamma$. Return is $G_t=\sum_{k\ge0}\gamma^kr_{t+k+1}$.
...
With rewards 1, 2, 3 and $\gamma=.9$, compute $G_0=1+.9(2)+.9^2(3)=5.23$.
...
A recommender’s immediate click reward can conflict with long-term satisfaction; choose a return and delayed feedback carefully.
...
In an offline dataset, actions reflect a behavior policy. Evaluating a new policy without coverage assumptions can be dangerously biased.
...
Reward definitions invite specification gaming; inspect trajectories and counterfactual harms, not just total return.

...
track: "deep-learning"
status: live
summary: "Multiple learned projections provide distinct relation subspaces, but heads require empirical validation."
...
Head $i$ computes $A_i=\operatorname{Attn}(QW_i^Q,KW_i^K,VW_i^V)$; concatenate then project: $\operatorname{Concat}(A_1,\dots,A_h)W^O$.
...
Splitting a fixed $d_{model}$ across $h$ heads keeps total width roughly constant; it does not automatically multiply information.
...
For `Paris is to France`, one head may focus on a relation while another tracks syntax; test ablations before narrating head roles.
...
In vision-language cross-attention, text queries image patches; a wrong crop can remove the evidence even if the attention code is correct.
...
During encoder-decoder decoding, cache encoder keys/values once; recomputing them per generated token wastes work.

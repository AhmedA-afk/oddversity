...
track: "deep-learning"
status: live
summary: "Policy optimization estimates how action probabilities affect return, while off-policy methods reuse data under additional assumptions."
...
REINFORCE uses $\nabla J(\theta)=\mathbb E[\nabla\log\pi_\theta(a\mid s)G_t]$. Subtracting a baseline keeps expectation while reducing variance.
...
If return is 10 and baseline is 7, advantage is 3; reinforce the sampled action in proportion to its positive advantage.
...
Importance ratio $\pi(a\mid s)/\mu(a\mid s)$ corrects behavior-policy data but can explode when actions lack coverage.
...
A bandit with sparse reward needs exploration; report regret and safety incidents, not only final average reward.
...
Do not deploy online exploration on people or critical systems without explicit constraints, monitoring, and rollback.

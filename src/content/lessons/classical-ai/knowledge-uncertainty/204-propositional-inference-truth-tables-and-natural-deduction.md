...
order: 204
status: live
summary: "Separate a statement that happens to be plausible from one that is entailed by the premises."
...
For premises `P → Q` and `P`, modus ponens derives `Q`; it does not permit deriving `P` from `Q`. A truth table exposes this asymmetry.
...
Inference rules preserve truth only when their premises are correctly scoped and their propositions have stable meanings.

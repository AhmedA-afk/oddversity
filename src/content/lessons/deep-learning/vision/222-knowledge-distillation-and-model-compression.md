...
status: live
order: 222
summary: "Compress visual models while preserving task, calibration, and slice-level requirements."
duration: "25 min read"
---

Distillation trains a student against labels and a teacher distribution:
(L=\alpha T^2 KL(p_T^T\Vert p_S^T)+(1-\alpha)CE(y,p_S)). Temperature (T>1)
softens logits; the (T^2) factor keeps gradient scale comparable. A teacher's
confusions can teach useful similarity, but also transfer bias and errors.

Scenario: a cloud teacher sends a 5M-parameter student to phones. Scenario: a
segmentation student matches mean IoU but loses tiny critical objects. Scenario: a
teacher trained on private data may leak membership or encode prohibited attributes.
Compression is a system-level trade, not an accuracy-only exercise.

```python
soft = F.kl_div(F.log_softmax(s/T, -1), F.softmax(t/T, -1), reduction="batchmean") * T*T
loss = 0.7 * soft + 0.3 * F.cross_entropy(s, y)
```

## Diagnostics and assignment

Compare teacher/student accuracy, NLL, ECE, per-class recall, latency, energy, and
peak memory. Inspect disagreement examples: students often fail rare or small classes
even when aggregate metrics look equal. Distil a small classifier at two temperatures,
calculate the loss for one logits pair, and submit an acceptance table for server,
phone, and offline-device conditions. State whether the student may inherit the
teacher's deployment authorization.


## Advanced studio: calculation, implementation, and decision evidence

The compact explanation above is only a starting point. To master **knowledge distillation and model compression**, make

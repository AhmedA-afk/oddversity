...
track: "deep-learning"
status: live
summary: "Sequence-to-sequence learning maps a source sequence to a target sequence with a conditional likelihood."
...
Optimize $-\sum_u\log p(y_u\mid y_{<u},x_{1:T})$. An encoder produces states; a decoder conditions on its history and source context.
...
A fixed-vector encoder creates an information bottleneck. Attention replaces one $c$ with $c_u=\sum_t\alpha_{ut}h_t$.
...
For translation, source `the bank` is ambiguous; the decoder needs neighboring source words to select financial versus river sense.
...
For speech recognition, input frames outnumber output tokens, so lengths differ and padding/masking must be separate on each side.
...
For summarization, a high likelihood output may copy a named entity incorrectly; validate factual support, not only n-gram overlap.
...
+
## Advanced laboratory: encoder-decoder alignment

Design a controlled experiment around source reversal and copy tasks. A model that cannot learn these synthetic controls is not ready for open-ended translation or summarization. Do not start with a full-scale model. First make a synthetic fixture where the correct behavior is known, calculate the expected result, and assert it in a test. Then repeat on a small real or realistically noisy slice. Keep the data contract explicit: what one record means, which fields are available at decision time, what constitutes a group or sequence boundary, and what information is forbidden. Save the seed, environment, input fingerprint, configuration, and the exact metric denominator with each result.

Compare fixed context, additive attention, and cross-attention at matched training budget. Report source-length slices and target-length slices. Check teacher-forced loss and free-running quality separately to expose exposure mismatch.

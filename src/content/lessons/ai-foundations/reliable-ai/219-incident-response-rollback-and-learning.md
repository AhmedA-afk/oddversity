...
order: 219
status: live
summary: "A rigorous, practice-led lesson on incident severity, containment, user communication, rollback, and postmortem practice for building AI systems that can be evaluated, challenged, and operated responsibly."
duration: "50–70 min"
updated: "2026-08-30"
---

# AI Incidents, Rollback, and Organisational Learning
...
available when a decision is made. This lesson examines **incident severity, containment, user communication, rollback, and postmortem practice**. The goal is not to
...
metric misleading here?** In this unit, the relevant mechanism is incident severity, containment, user communication, rollback, and postmortem practice. Treat it
...
For ai incidents, rollback, and organisational learning, use the same discipline with the relevant signals:
time-to-detect, time-to-contain, affected users, recurrence rate, and action-item closure. Estimate uncertainty rather than treating one run as final. For a proportion,
...
Consider a chatbot gives prohibited advice after a knowledge-base update. Begin with a narrow workflow map: who supplies an input, what
...
The characteristic failure to avoid is **writing blame-oriented postmortems that hide the system conditions that enabled failure**. It often survives a dashboard
...
4. Compute time-to-detect, time-to-contain, affected users, recurrence rate, and action-item closure; publish slice counts and uncertainty.

...
order: 212
status: live
summary: "Build auditable rule-based decisions while controlling which eligible rule fires when several match."
...
If two rules recommend conflicting actions, a salience policy, specificity rule, or human escalation path must resolve the conflict. There is no neutral default.
...
A rule engine can reproduce inconsistent policy at speed. Version the rules, test changes, and provide an appeal path.

...
Connect local connectivity and equivariance to exact convolution shapes and practical image pipelines. Treat this lesson as a working session: calculate on paper first, then make the smallest program that can falsify your reasoning. A deep-learning implementation is credible only when its tensor contracts, objective, and measurements agree.

## Core derivation

For 2D convolution, output spatial size is floor((N+2P−D(K−1)−1)/S)+1. Weight sharing makes a translated input produce translated feature maps (before pooling/boundaries), not full invariance.
...
1) Compute 32×32 with K=3,P=1,S=1. 2) Compute a stride-2 result. 3) Count conv parameters. 4) Contrast translation equivariance with rotation invariance.
...
Implement shape assertions for a conv stack and train a small image model with/without augmentation. Inspect a deliberately off-by-one padding bug. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Confusing channels-last and channels-first conventions; write the tensor layout at every boundary.
...
For N=32,K=3,P=1,S=1 output is 32; with S=2 it is 16. A 3→64 K=3 convolution has 3×64×3×3+64=1,792 parameters. Dilation 2 with K=5 has effective width 9. Conveyor inspection can reasonably assume small translation equivariance but not arbitrary rotation. Debug channels-first/last confusion and residual off-by-one sizes by testing non-square inputs and explicit B,C,H,W assertions.
...
For **Convolutional Inductive Biases and Shape Accounting**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.

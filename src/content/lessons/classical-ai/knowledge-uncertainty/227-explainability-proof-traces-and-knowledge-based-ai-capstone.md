...
order: 227
status: live
summary: "Produce an explanation that matches the system’s actual reasoning, uncertainty, and limits instead of a persuasive after-the-fact story."
...
Produce an explanation that matches the system’s actual reasoning, uncertainty, and limits instead of a persuasive after-the-fact story. Knowledge-based AI is strongest when it makes its representation and uncertainty explicit: a reader can inspect what the system was told, which inference step it took, and which assumption would change the recommendation. That is a different promise from a system that only produces a score. It is especially valuable where a decision must be reviewed, justified, or revised as new evidence arrives.
...
A rule trace may show facts, fired rules, and priority decisions; a Bayesian explanation may show prior, likelihood, posterior, and sensitivity to assumptions.
...
An explanation can be accurate yet unusable, or simple yet misleading. Test it with the people who must act on it and preserve an appeal path.

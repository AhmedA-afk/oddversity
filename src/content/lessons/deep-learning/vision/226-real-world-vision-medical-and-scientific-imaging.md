...
status: live
order: 226
summary: "Design clinically and scientifically honest visual models under shift, uncertainty, and annotation limits."
duration: "29 min read"
---

Medical/scientific images mix physics, acquisition protocol, and expert interpretation.
Train/test splits by image can leak patient, specimen, slide, or institution identity;
split by the highest relevant unit. A lesion classifier with 95% image accuracy may
fail if all images from one patient appear in both partitions.

If prevalence is 1%, sensitivity 90%, specificity 90%, then among 10,000 cases:
90 true positives and 990 false positives give PPV (90/(90+990)=8.3\%).
This base-rate calculation changes clinical workflow even when ROC-AUC is impressive.

Scenario: a segmentation boundary is uncertain because experts disagree. Scenario:
scanner/site artifacts become shortcuts. Scenario: a prospective deployment sees a
new protocol and requires abstention, not forced classification.

```text
patient/site split -> locked evaluation -> prospective shadow run -> monitored clinical workflow
```

## Assignment

Write a protocol for a triage assist tool: intended use, excluded cases, reference
standard, patient-level split, calibration endpoint, and false-negative escalation.
Calculate PPV/NPV at two prevalences. Review 15 errors with a domain expert rubric and
separate annotation disagreement from model error. Conclude with a statement the model
is permitted to make—and three it is not.


## Advanced studio: calculation, implementation, and decision evidence

The compact explanation above is only a starting point. To master **real world vision medical and scientific imaging**, make

...
track: "deep-learning"
status: live
summary: "GRUs trade LSTM’s separate cell state for fewer gates and often competitive performance."
...
A common form is $z_t=\sigma(W_z[x_t,h_{t-1}])$, $\tilde h_t=\tanh(W_h[x_t,r_t\odot h_{t-1}])$, $h_t=(1-z_t)h_{t-1}+z_t\tilde h_t$.
...
The update gate yields an additive interpolation; $z=0$ copies history and $z=1$ overwrites it. This is a learnable timescale.
...
For $h_{t-1}=2$, $\tilde h_t=-1$, $z=.25$, calculate $h_t=1.25$; the state changes gradually.
...
On a small clinical time-series dataset, compare GRU and LSTM at fixed parameter budget rather than declaring one universally better.
...
If validation improves only after increasing hidden size, report parameter count and latency: the gate family may not be the cause.

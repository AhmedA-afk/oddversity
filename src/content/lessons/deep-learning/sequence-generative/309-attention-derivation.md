...
track: "deep-learning"
status: live
summary: "Attention is a differentiable weighted retrieval operation, with scaling that keeps logits numerically usable."
...
For query $q$, keys $K$, values $V$: $\alpha=\operatorname{softmax}(qK^\top/\sqrt{d_k})$, $c=\alpha V$. Softmax makes weights nonnegative and sum to one.
...
If query/key components have variance one, $q\cdot k$ variance grows with $d_k$; dividing by $\sqrt{d_k}$ prevents saturation.
...
Let logits be $[2,0,-1]$. Softmax is approximately $[.844,.114,.042]$; use these weights to compute a weighted value average by hand.
...
Cross-attention lets a decoder position attend to an encoder’s source tokens; self-attention lets each position mix peer context.
...
Attention weights can be sharp yet wrong: attention is a routing computation, not automatically a faithful explanation.

...
order: 206
status: live
summary: "Use resolution to show that a conclusion follows by demonstrating that its negation conflicts with the knowledge base."
...
From `P ∨ Q` and `¬P ∨ R`, resolution derives `Q ∨ R`. If adding `¬goal` eventually produces the empty clause, the original goal is entailed.
...
Resolution can grow a large search space. Record clause provenance so a proof is inspectable rather than merely asserted.

...
order: 202
status: live
summary: "Decide what absence of a statement means before an agent treats it as evidence."
...
In a closed customer database, failure to find `paid(invoice_42)` may justify a provisional “unpaid” state. In an open clinical record, missing `allergy(lee, penicillin)` is not evidence of no allergy.
...
Default conclusions must carry their assumptions; otherwise later evidence looks like an inconsistency rather than a revision.

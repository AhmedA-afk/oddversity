...
track: "deep-learning"
status: live
summary: "A vanilla RNN is a compact baseline whose failures teach sequence modelling."
...
Use $h_t=\tanh(W_xx_t+W_hh_{t-1}+b)$ and logits $o_t=W_oh_t$. Reset state between independent examples unless state continuity is intended.
...
Teacher forcing supplies the true previous target during training; autoregression supplies the model output at inference, creating exposure mismatch.
...
For a next-character model, verify target alignment: input `hell` should predict `ello`, not copy `hell`.
...
For streaming anomaly detection, carry state across adjacent windows but reset it at machine boundaries to avoid cross-machine leakage.
...
A bidirectional RNN improves tagging only when future context is legal; it is invalid for real-time forecasting.

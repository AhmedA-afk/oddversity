...
Attention(Q,K,V)=softmax(QKᵀ/√d_k+M)V. A causal mask has −∞ above the diagonal, ensuring position t cannot attend to positions >t. Padding masks remove invalid tokens.
...
1) Calculate one 2-token attention row. 2) Apply a causal mask. 3) Show why divide by √d_k. 4) Contrast encoder and decoder masks.
...
Write pseudocode for masked attention; test that changing a future token cannot alter earlier outputs. Log attention-row sums. Keep one seed-fixed smoke test and one deliberately adversarial test. Inspect one batch end-to-end before launching a long run.
...
Masking after softmax rather than before, or forgetting to renormalise; use a large negative additive mask on logits.
...
For q=[1,0], keys [[1,0],[0,1]], values [[2,0],[0,4]], scaled scores yield weights near [.67,.33] and output [1.34,1.32]. A first-position causal mask forces output [2,0]. With dk=64 divide by 8, preventing extreme score scale. Next-token products must prove future changes cannot affect earlier outputs. Debug applying masks after softmax and confusing padding/causal masks; test row sums and future-token invariance.
...
For **Sequences, Attention, and Causal Masking**, repeat the numerical studio with three deliberate changes: one change that should preserve the result, one that should change it in a predictable direction, and one boundary case that should trigger an assertion or expose a limitation. State the expected outcome before running the code. Then record the actual value, tensor shape, dtype, and any difference from the prediction.
